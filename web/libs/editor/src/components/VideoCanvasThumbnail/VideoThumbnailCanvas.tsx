...
import { MAX_ZOOM, MIN_ZOOM } from './VideoThumbnailConstants';
import { VirtualCanvas } from './VirtualThumbnailCanvas';
import { VirtualVideo } from './VirtualVideoThumbnail';
...
export const VideoCanvas = memo(forwardRef<VideoRef, VideoProps>((props, ref) => {
  ...
  return (
    <Block ref={rootRef} name="video-canvas-thumb" className={`${props.isCurrent ? 'selected' : ''}`}>
      {loading && (
        <Elem name="loading">
          <Block name="spinner"/>
        </Elem>
      )}
      {props.title && <span style={{ display: 'block', fontSize: '0.75rem', fontWeight: 'bold', margin: 'auto 0.45rem' }}>{props.title}</span>}
      <Elem
        name="view"

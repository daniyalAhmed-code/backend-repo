...
export const AnnotationButton = observer(({ entity, capabilities, annotationStore, onAnnotationChange }: AnnotationButtonInterface) => {
  ...
  const isAdmin = isAllowed(annotationStore.store.user?.email);

  const shouldBeDisabled = !annotationStore.store.hasAdminAccess && !(entity.user.email === annotationStore.store.user?.email);//isAdmin ? false : !(entity.user.email === annotationStore.store.user?.email);

useEffect(() => {
  // const selectedAnn = annotationStore.selected;
  if (!annotationStore.store.hasAdminAccess && entity.user.email === annotationStore.store.user?.email) {
    // const currEntityActivity = entity.
    const selectedAnn = annotationStore.selected;
    const isAlreadyUsersSelected = selectedAnn.user.email === annotationStore.store.user?.email;
    if(!isAlreadyUsersSelected) annotationStore.selectAnnotation(entity.id);
  }
}, []);
  ...
  const ContextMenu = ({ entity, capabilities, disabled }: AnnotationButtonInterface) => {
    ...
    return (
      <Block name="AnnotationButtonContextMenu" style={disabled ? { pointerEvents: 'none', position: 'relative' } : {}}>
        { disabled && <div style={{ position: 'absolute', content: "''", top: 0, bottom: 0, right: 0, left: 0, backgroundColor: '#BFBFBF', zIndex: 999, opacity: '0.325' }}></div>}
        {showGroundTruth && (
  ...
  return (
    <Block name='annotation-button' mod={{ selected: entity.selected, contextMenuOpen: isContextMenuOpen }} style={shouldBeDisabled ? { cursor: 'not-allowed' } : {}}>
      ...
      <Elem name='contextMenu'>
        <Dropdown.Trigger
          content={<ContextMenu entity={entity} capabilities={capabilities} annotationStore={annotationStore} disabled={shouldBeDisabled}/>}

...
export class Regions {
  ...
  redraw(force=false) {
    this.visualizer.draw(!force, force);
    // this.visualizer.draw(true, force);
  }
  ...
  renderAgain() {
    ...
    this.regions?.forEach(itm => this.removeRegion(itm.id, false));
    this.addRegions([...regionsCopy] as unknown as RegionOptions[])
    // this.regions?.forEach(itm => itm?.invoke('click', [itm, {} as unknown as MouseEvent]))
    // console.log('*-*-*-*--*-*-*-*-*-*-*-**-*-*last', last);
    if(this.regions?.length) {
      const last = this.regions[this.regions.length - 1];
      last.invoke('click', [last, {} as unknown as MouseEvent]);
    }
  }

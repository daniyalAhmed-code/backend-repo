...
export class Waveform extends Events<WaveformEventTypes> {
  ...
  scrollHalfVisibleTime(backwards = false) {
    const { width } = this.visualizer;
    const scrollLeftPx = this.visualizer.getScrollLeftPx();

    // Calculate the viewable duration based on the width of the visualizer
    const viewableDuration = this.mapToTime(width);
    const [interval] = this.timeline.getIntervals(viewableDuration);

    // Calculate the exact start time based on current scroll position
    const exactStart = this.mapToTime(Math.abs(scrollLeftPx));
    const segmentStart = Math.floor(exactStart / interval) * interval;
    const segmentEnd = segmentStart + viewableDuration;

    // Calculate half of the current visible duration
    const halfVisibleDuration = (segmentEnd - segmentStart) / 2;

    // Calculate the new segment start time
    const newSegmentStart = backwards ? segmentStart - halfVisibleDuration : segmentStart + halfVisibleDuration;
    const newSegmentEnd = newSegmentStart + viewableDuration;

    if (backwards && newSegmentStart < 0) {
      const newScrollLeft = 0;
      if (newScrollLeft !== this.visualizer.getScrollLeft()) {
        this.visualizer.setScrollLeft(newScrollLeft);
        this.invoke('scroll', [newScrollLeft]);
      }
      return;
    }

    // Check if the new start time is valid
    if (newSegmentEnd >= this.duration) {
      const lastStart = this.duration - (segmentEnd - segmentStart);
      const newScrollLeftPx = this.mapToPx(lastStart);
      const newScrollLeft = newScrollLeftPx / this.visualizer.fullWidth;
      if (newScrollLeft !== this.visualizer.getScrollLeft()) {
        this.visualizer.setScrollLeft(newScrollLeft);
        this.invoke('scroll', [newScrollLeft]);
      }
      return; // Do not update if the start time exceeds or equals the duration
    }

    // Calculate the new scroll position in pixels
    const newScrollLeftPx = this.mapToPx(newSegmentStart);
    const newScrollLeft = newScrollLeftPx / this.visualizer.fullWidth;

    // Update the visualizer's scroll position
    if (newScrollLeft !== this.visualizer.getScrollLeft()) {
      this.visualizer.setScrollLeft(newScrollLeft);
      this.invoke('scroll', [newScrollLeft]);
    }

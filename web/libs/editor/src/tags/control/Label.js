...
import { getType, onSnapshot, types } from 'mobx-state-tree';
...
const TagAttrs = types.model({
  ...
  isHotKeySelected: types.optional(types.boolean, false),
...
const Model = types.model({
   ...
   * @param {boolean} value
   */
  setIsHotKeySelected(value) {
    self.isHotKeySelected = value;
  },

  onHotKey() {
    if (!self.selected) self.isHotKeySelected = true;
    else if (!self.selected && self.isHotKeySelected) {
      self.isHotKeySelected = false;
      self.isHotKeySelected = true;
    }
    else if (self.selected && self.isHotKeySelected) self.isHotKeySelected = false;
...
const HtxLabelView = inject('store')(
  observer(({ item, store }) => {
    ...
    useEffect(() => {
      if(item.isHotKeySelected) {
        // store.setSelectedLabels([{ value: item._value, isAborted: false, isCompleted: false }]);
        const isPresent = store.selectedLabels?.some(itm => itm?.value === item._value);
        // if (!isPresent) store.setSelectedLabels([{ value: item._value, isAborted: false, isCompleted: false }]);
        if (!isPresent) store.pushToSelectedLabels({ value: item._value, isAborted: false, isCompleted: false });
      }
      else {
        store?.updateInSelectedLabelsByValue( item._value ,{ value: item._value, isAborted: false, isCompleted: true });
        // const isPresent = store.selectedLabels?.some(itm => itm?.value === item._value);
        // if (isPresent) item.setIsHotKeySelected(true);
      }
    }, [item.isHotKeySelected]);

    useEffect(() => {
      if(item.isClicked){
        store.selectedSegments?.forEach(itm => {
          item.onClick?.();
          const jsonObj = JSON.parse(itm);
          store.removeFromSelectedSegments(jsonObj.id);
        });
      }
    }, [item.isClicked]);

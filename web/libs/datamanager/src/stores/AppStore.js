import { cast, destroy, flow, getSnapshot, types } from "mobx-state-tree";
...
import { fetchAdminsList, isAllowed } from "../../../../RBAC";
...
export const AppStore = types
  .model("AppStore", {
    ...
    projectStats: types.optional(types.string, '{}'),
  })
  .views((self) => ({
  ...
  .actions((self) => ({
    ...
    afterCreate() {
      ...
      self.SDK?.api?.call('me').then(res => self.auth = JSON.stringify(res));
      if(!self.adminsList.length) {
        self.adminsListLoading.flag = true;
        // setAdminsList
        fetchAdminsList(this.setAdminsListLoading, this.setAdminsList);
      }
    },
    ...
    setAdminsListLoading(newVal=false) {
    ...
    fetchProjectStats: flow(function* (options = {}) {
      const params =
        options && options.interaction
          ? {
            interaction: options.interaction,
      ...
      try {
        const projectStats = yield self.apiCall("projectStats", params);

        self.projectStats = JSON.stringify(projectStats);
      } catch {
        self.crash();
        return false;
      }
    ...
    fetchUsers: flow(function* () {
      ...
      const tempList = list.map(u => ({ ...u, has_admin_access: isAllowed(u.email) }));
      const currAuth = yield self.apiCall('me');
      self.auth = JSON.stringify({ ...currAuth, has_admin_access: currAuth.has_admin_access ? true : isAllowed(currAuth.email)});
      self.users.push(...(tempList));
    }),

...
export const ActionsButton = injector(observer(({ store, size, hasSelected, ...rest }) => {
  ...
  const actions = store.availableActions
    ...
    onSnapshot(store.adminsListLoading, sna => {
      setLoadingAdmins(sna.flag);
    });

    const fetchMe = async () => {
      setLoadingAdmins(true);
      try{
        const me = await store._sdk?.api?.me?.();
      if (me) setAuth(me)
      }
      catch(err){}
      finally{
    ...
    useEffect(() => {
      if (auth) {
        actions.map((act, idx) => {
          if(act.id?.includes('delete')) {
            // act.setShouldBeDisabled?.(!isAllowed(auth.email, store.adminsList))
            act.setShouldBeDisabled?.(!auth?.has_admin_access && !isAllowed(auth?.email))
          }
        })
      }
    }, [auth]);
    ...
    const titleContainer = (
      <Block 
        ...
        mod={{ 
          ...
          disabled: action.disabled || action.shouldBeDisabled,
        }}
        name='actionButton'
      >

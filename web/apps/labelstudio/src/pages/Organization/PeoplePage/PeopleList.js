...
export const PeopleList = ({ onSelect, selectedUser, defaultSelected, updatedUser, loading }) => {
  const api = useAPI();
          ...
          {usersList ? (
            <Elem name="users">
              <Elem name="header">
                ...
                <Elem name="column" mix="last-activity">User Role</Elem>
              </Elem>
              <Elem name="body">
                {usersList.map(({ user }) => {
                  ...
                  return (
                    <Elem key={`user-${user.id}`} name="user" mod={{ active }} onClick={() => selectUser(user)}>
                      ...
                      <Elem name="field" mix="last-activity">
                        {isAllowed(user.email) ? 'Super Admin' : user.has_admin_access ? 'Admin' : 'Normal User' }
                      </Elem>
                    </Elem>
                  );
                })}

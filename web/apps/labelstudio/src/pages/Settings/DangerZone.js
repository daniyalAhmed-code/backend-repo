...
export const DangerZone = () => {
  ...
  const { store: appStore, fetchAdminUsers } = useAppStore();
  const { adminsList } = appStore;
  ...
  useEffect(() => {
    if (!adminsList.length) {
      setAdminListLoading(true);
      fetchAdminUsers(() => setAdminListLoading(false));
    } else {
      setAdminListLoading(false);
    }
  }, []);
  ...
  const buttons = useMemo(() => [{
    ...
    disabled: !user?.has_admin_access && !isAllowed(user?.email),//(adminListLoading || !isAllowed(user?.email, adminsList)),
  }], [project, user, adminsList, adminListLoading]);
